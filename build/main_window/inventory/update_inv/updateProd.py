...
class updateInventory(Frame):
    def __init__(self, parent, controller=None, *args, **kwargs):
        ...
        entry_3 = Entry(
            self,
            textvariable=self.data["descrpt"],
            bd=0,
            bg="#A8A1A1",
            fg="#000716",
            highlightthickness=0
    ...
    def initialize(self):
        ...
        if self.inventory_data is not None:
            self.selected_inventory_data = list(
                filter(lambda x: str(x[0]) == self.selected_r_id, self.inventory_data)
            )

            if self.selected_inventory_data:
                self.selected_inventory_data= self.selected_inventory_data[0]


                self.canvas.itemconfigure(self.id_text, text=self.selected_inventory_data[0])
                self.data["p_name"].set(self.selected_inventory_data[1])
                self.data["descrpt"].set(self.selected_inventory_data[2])
                self.data["qty"].set(self.selected_inventory_data[3])
                self.data["cost"].set(self.selected_inventory_data[4])
                self.data["measure"].set(self.selected_inventory_data[5])
           
           
      

    def handle_update(self):
        result = db_controller.update_inventory(
            self.selected_r_id,
            p_name=self.data["p_name"].get(),
            descp=self.data["descrpt"].get(),
            qty=self.data["qty"].get(),
            cost=self.data["cost"].get(),  
            measure=self.data["measure"].get(),    
